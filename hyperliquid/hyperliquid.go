...
func NewFuturesClient(privateKeyHex string, isTestnet bool) (*FuturesClient, error) {
	...
	// Вычисляем адрес аккаунта из приватного ключа
	accountAddr := crypto.PubkeyToAddress(privateKey.PublicKey).Hex()

	// Создаем Exchange клиент
	exchange := hyperliquid.NewExchange(
		privateKey,
		baseURL,
		nil, // meta
		"",  // vaultAddr (пустой для обычных аккаунтов)
		accountAddr,
		nil, // spotMeta
	)

	// Создаем Info клиент
	info := hyperliquid.NewInfo(baseURL, false, nil, nil)
...
// AccountAddress возвращает адрес аккаунта
func (c *FuturesClient) AccountAddress() string {
