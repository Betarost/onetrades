...
// Do выполняет изменение ордера (отмена старого и создание нового)
func (r *futures_amendOrder) Do(ctx context.Context) ([]entity.PlaceOrder, error) {
	if r.symbol == "" {
	...
	if r.orderId != "" {
		...
		orderQueryResult, err = r.client.info.QueryOrderByOid(r.client.AccountAddress(), orderIdInt)
	} else if r.clientOrderId != "" {
		// Запрашиваем ордер по клиентскому ID
		orderQueryResult, err = r.client.info.QueryOrderByCloid(r.client.AccountAddress(), r.clientOrderId)
	} else {
	...
	// Step 2: Создаем новый ордер с обновленными параметрами
	newOrderRequest := r.client.NewPlaceOrder().
		Symbol(r.symbol).
		Side(convertSide(string(originalOrder.Side))).
		Type(convertOrderType("limit")) // Предполагаем лимитный ордер
	...
	// Генерируем новый клиентский ID
	newClientOrderId := r.clientOrderId + "_amended"
	if r.clientOrderId == "" {
		newClientOrderId = "amended_" + strconv.FormatInt(time.Now().UnixNano(), 10)
	}
